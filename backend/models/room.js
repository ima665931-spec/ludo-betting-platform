...
const STAKE_LEVELS = {
    beginner: { entryFee: 10, label: 'Beginner' },
    amateur: { entryFee: 50, label: 'Amateur' },
    pro: { entryFee: 100, label: 'Pro' },
    legend: { entryFee: 500, label: 'Legend' },
};
...
const RoomSchema = new mongoose.Schema({
    ...
    // --- Betting fields ---
    stakeLevel: { type: String, enum: Object.keys(STAKE_LEVELS), default: 'beginner' },
    ...
    playerUserIds: [{ type: mongoose.Schema.Types.ObjectId, ref: 'User' }],
    // ---
    pawns: {
        type: [PawnSchema],
        default: () => {
...
RoomSchema.methods.getPrizePool = function () {
    return this.prizePool;
};

RoomSchema.methods.getWinnerUserId = function () {
    const winnerColor = this.winner;
    const winnerPlayer = this.players.find(p => p.color === winnerColor);
    if (!winnerPlayer) return null;
    const idx = this.players.indexOf(winnerPlayer);
    return this.playerUserIds[idx] || null;
};

RoomSchema.methods.collectEntryFee = function () {
...
module.exports = Room;
module.exports.STAKE_LEVELS = STAKE_LEVELS;
